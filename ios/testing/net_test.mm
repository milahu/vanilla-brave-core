...
// AmazonRootCA1
// From: https://github.com/brave/brave-core/blob/master/chromium_src/net/tools/transport_security_state_generator/input_file_parsers.cc
...
+ (nullable SecCertificateRef)getCertificate {
    ...
    cert_string = [cert_string stringByReplacingOccurrencesOfString:@"\t" withString:@""];
    cert_string = [cert_string stringByReplacingOccurrencesOfString:@"\r" withString:@""];
    cert_string = [cert_string stringByReplacingOccurrencesOfString:@"\n" withString:@""];
    
    NSData* data = [[NSData alloc] initWithBase64EncodedString:cert_string
...
- (void)testAcceptableCertificates {
    std::size_t acceptable_certs_count =
        sizeof(net::kBraveAcceptableCerts) /
          sizeof(net::kBraveAcceptableCerts[0]) - 1;
    
    NSArray<NSData*>* certs = [BraveCertificateUtility acceptableCerts];
...
- (void)testHashCertificateSPKI {
    std::string amazon_root_ca1_spki_hash = std::string(net::kSPKIHash_AmazonRootCA1);
    XCTAssertTrue(amazon_root_ca1_spki_hash.size() > 0);
    
    NSData* amzn_spki_hash_data = [NSData dataWithBytes:&amazon_root_ca1_spki_hash[0] length:amazon_root_ca1_spki_hash.size()];
    XCTAssertNotNil(amzn_spki_hash_data);
    XCTAssertTrue([amzn_spki_hash_data length] > 0);
    
    NSData* spki_hash = [BraveCertificateUtility hashCertificateSPKI:[NetTest getCertificate]];
